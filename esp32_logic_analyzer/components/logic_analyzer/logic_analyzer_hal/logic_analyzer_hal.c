...
#endif
#ifdef CONFIG_IDF_TARGET_ESP32P4
    esp_cache_msync(la_frame.fb.buf, la_frame.fb.len, ESP_CACHE_MSYNC_FLAG_DIR_M2C);
#endif
...
esp_err_t start_logic_analyzer(logic_analyzer_config_t *config)
{
    ...
    else
    {
        ...
        // PSRAM - only esp32s3/P4 - 8/16 channels
    ...
    // Classic ESP32 DMA buffers in internal RAM do not need cache sync here.
